...
class L2TiledLevel extends TiledMap {
	...
	public function loadObjects(state:L2PlayState){
	...
	private function loadObject(o:TiledObject, g:TiledObjectGroup, state:L2PlayState) {
		...
		switch (o.type.toLowerCase()){
			...
			case "player_start":
				var p1 = new L2Player(x, y);
				state.p1 = p1;
				state.add(p1);

			case "exit":
				state.exits.add(new L2Exit(x, y)); // for a single, generic exit

			case "powerup":
				state.powerUps.add(new L2PowerUp(x, y));
			...
			case "enemy_start":
				...
				state.enemies.add(new L2Enemy(x, y, speedX, state));
		}
	}
