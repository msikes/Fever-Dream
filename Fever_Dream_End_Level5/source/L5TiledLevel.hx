...
class L5TiledLevel extends TiledMap {
	...
	public function loadObjects(state:L5PlayState){
	...
	private function loadObject(o:TiledObject, g:TiledObjectGroup, state:L5PlayState) {
		...
		switch (o.type.toLowerCase()){
			...
			case "player_start":
				var p1 = new L5Player(x, y);
				state.p1 = p1;
				state.add(p1);

			case "exit":
				state.exits.add(new L5Exit(x, y)); // for a single, generic exit

			case "powerup":
				state.powerUps.add(new L5PowerUp(x, y));
			...
			case "enemy_start":
				...
				state.enemies.add(new L5Enemy(x, y, speedX, state));
		}
	}
