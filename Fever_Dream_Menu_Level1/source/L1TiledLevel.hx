...
class L1TiledLevel extends TiledMap {
	...
	public function loadObjects(state:L1PlayState){
	...
	private function loadObject(o:TiledObject, g:TiledObjectGroup, state:L1PlayState) {
		...
		switch (o.type.toLowerCase()){
			...
			case "player_start":
				var p1 = new L1Player(x, y);
				FlxG.camera.follow(p1);
				state.p1 = p1;
				state.add(p1);

			case "exit":
				state.exits.add(new L1Exit(x, y)); // for a single, generic exit

			case "powerup":
				state.powerUps.add(new L1PowerUp(x, y));
