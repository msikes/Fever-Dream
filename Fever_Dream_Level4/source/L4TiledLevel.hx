...
class L4TiledLevel extends TiledMap {
	...
	public function loadObjects(state:L4PlayState){
	...
	private function loadObject(o:TiledObject, g:TiledObjectGroup, state:L4PlayState) {
		...
		switch (o.type.toLowerCase()){
			...
			case "player_start":
				var p1 = new L4Player(x, y);
				state.p1 = p1;
				state.add(p1);

			case "exit":
				state.exits.add(new L4Exit(x, y)); // for a single, generic exit

			case "powerup":
				state.powerUps.add(new L4PowerUp(x, y));
			...
			case "enemy_start":
				...
				state.enemies.add(new L4Enemy(x, y, speedX, state));
		}
	}
